...
def nms(bboxes, iouThreshold):
    ...
    while len(bboxes) > 0:
        ...
        filtered_bboxes = [bbox for i, bbox in enumerate(
            bboxes) if iou_values[i] <= iouThreshold]
        bboxes = filtered_bboxes
    return selected_bboxes


def calculate_iou(bbox1, bbox2):
    x1, y1, w1, h1 = bbox1[0], bbox1[1], bbox1[2] - \
        bbox1[0], bbox1[3] - bbox1[1]
    x2, y2, w2, h2 = bbox2[0], bbox2[1], bbox2[2] - \
...
def visualizeBbox(img, bbox, labelEncoder):
    ...
    if bbox != []:
        ...
        result_directory = os.path.join(current_directory, 'static', 'result')
        timestamp = int(time.time())
        filename = f"result_image_{timestamp}.jpg"
        imgDir = os.path.join(result_directory, filename)
        print('Image saved at:', imgDir)
...
def predict(clf, scaler, image, labelEncoder):
    ...
    print(bbox)
    return visualizeBbox(image, bbox, labelEncoder)
